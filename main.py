...
class Application(tk.Frame):
    def __init__(self, master):
        ...
        self.master.title("Keyboard Control")
        ...
        self.key_state = {"W": 0, "A": 0, "S": 0, "D": 0, "Q": 0, "E": 0,
                          "Z": 0, "X": 0, "C": 0, "V": 0, "F": 0, "G": 0}

    def widget(self):
        # top
        self.top_label = tk.Label(self.frame_list[0], text="ニコニコcontrol (shift+) WASD, QE, ZXCVFG", width=40, font=self._font)  # place top label into label[0]
        strvar = tk.StringVar(value='192.168.0.1')
    ...
    def connect_rc(self):
        ...
        self.is_connect_rob = self.rob.connect(ip=ip_str, rc_type=rc_type, name='keyboardctrl')
        if self.is_connect_rob:
            self._writelog('connected succesfully')
            rob_info = self.rob.get_base_info() #BUGI
    ...
    def _timer_func(self):
        ...
        if self.is_running:
            ...
            if self.key_state["A"]: deviation[1] += self.rate_tomove
            if self.key_state["S"]: deviation[0] -= self.rate_tomove
            if self.key_state["D"]: deviation[1] -= self.rate_tomove
            if self.key_state["W"]: deviation[0] += self.rate_tomove
            if self.key_state["Q"]: deviation[2] += self.rate_tomove
            if self.key_state["E"]: deviation[2] -= self.rate_tomove
            if self.key_state["Z"]: deviation[3] += self.rate_tomove
            if self.key_state["X"]: deviation[3] -= self.rate_tomove
            if self.key_state["C"]: deviation[4] += self.rate_tomove
            if self.key_state["V"]: deviation[4] -= self.rate_tomove
            if self.key_state["F"]: deviation[5] += self.rate_tomove
            if self.key_state["G"]: deviation[5] -= self.rate_tomove

            for i in range(6):
                if not self.active_move[i].get():
                    deviation[i] = 0
